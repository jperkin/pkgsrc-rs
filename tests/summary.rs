/*
 * Comprehensive tests for the Summary module
 */
...
"#;
        ...
        let result: std::result::Result<Summary, _> = incomplete.parse();
...
"#;
        ...
        let result: std::result::Result<Summary, _> = invalid.parse();
        assert!(result.is_err());
    }
...
"#;
    ...
    fn test_edge_cases() {
        ...
        let _result: std::result::Result<Summary, _> = with_whitespace.parse();
        ...
        // Test empty input
        let _empty: std::result::Result<Summaries, _> = "".parse();
...
SIZE_PKG=1000"#,
    ...
    fn make_summary(name: &str, version: &str) -> Summary {
        SummaryBuilder::new()
            .pkgname(&format!("{}-{}", name, version))
            .comment("Test")
            .categories("devel")
            .description(vec!["Test package"])
            .machine_arch("x86_64")
            .opsys("Linux")
            .os_version("5.15")
            .pkgpath(&format!("devel/{}", name))
            .pkgtools_version("20091115")
            .size_pkg(1000)
            .build_date("2024-01-01")
            .build()
